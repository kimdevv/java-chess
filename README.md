...
### 방 이름 (Name)
- [x] 방 이름은 영어 소문자로만 이루어지며, 최대 10자이다. 공백일 수 없다.
- [x] 방 하나와 게임 하나는 1:1로 매핑된다.

### DB 관련
- [x] 방 이름을 통해 게임을 저장하고 불러온다.
- [x] 게임을 종료하더라도 이어서 플레이할 수 있도록 한다
- [x] 사용자의 `END` 입력 시 게임을 저장하고 종료한다.
- [x] 사용자의 `RESET` 입력 시 저장된 게임을 삭제하고 프로그램을 종료한다.
